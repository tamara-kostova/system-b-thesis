...
with tab_pending:
    ...
    try:
        submitted_permits = requests.get(
            f"{PERMIT_SERVICE_URL}/permits", params={"state": "submitted"}
        ).json()
        under_review_permits = requests.get(
            f"{PERMIT_SERVICE_URL}/permits", params={"state": "under_review"}
        ).json()
    ...
    if not pending:
        ...
        for p in pending:
            with st.expander(
                f"📋 {p['permit_id'][:8]}... | {p['holder']} | {p['purpose']} | {p['state']}"
            ):
                ...
                with col2:
                    ...
                    if p["state"] == "under_review":
                        ...
                        valid_from = st.date_input(
                            "Valid from", value=date.today(), key=f"vf_{p['permit_id']}"
                        )
                        ...
                        comment = st.text_area(
                            "Reason for refusal", key=f"comment_{p['permit_id']}"
                        )
...
with tab_all:
    ...
    for p in all_permits:
        ...
        st.write(
            f"{icon} `{p['permit_id'][:8]}` | **{p['holder']}** | {p['purpose']} | {p['state']} | {p['created_at'][:10]}"
        )
