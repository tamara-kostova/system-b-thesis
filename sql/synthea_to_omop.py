...
def transform_persons(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["ethnicity_concept_id"] = (
        df["ETHNICITY"]
        .str.lower()
        .map({"nonhispanic": 38003564, "hispanic": 38003563})
        .fillna(UNKNOWN)
        .astype(int)
    )
...
def transform_drugs(df: pd.DataFrame, id_map: dict, enc_map: dict) -> pd.DataFrame:
    ...
    out["drug_exposure_end_date"] = (
        pd.to_datetime(df["STOP"]).dt.date if "STOP" in df.columns else None
    )
    ...
    _register_concept(
        DRUG_TYPE_RX, "Prescription written", "Type Concept", "Type Concept", "Type Concept"
    )
    return out
...
def transform_observations(df: pd.DataFrame, id_map: dict, enc_map: dict) -> pd.DataFrame:
    ...
    _register_concept(
        MEAS_TYPE_EHR,
        "Test ordered through EHR order entry",
        "Type Concept",
        "Type Concept",
        "Type Concept",
    )
